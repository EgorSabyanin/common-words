...
1. [cheat sheet bem visual](https://9elements.com/bem-cheat-sheet/)


## Советы в использовании имен

Если произошел выбор в пользу одного слова, то оно должно использоваться везде.

*Например*, если выбран был title, то heading, не применяется и следующие имена формируются в порядке block__title, block2__title, block3__title.
...
`cards`, `card` - карточки с товаром

`products`, `product`, `basket` - продукты, продукт, корзина 

`sale`, `price`, `quantity` - скидка, цена, количество

`certificates`, `certificate`, `gifts`, `gift` - сертификаты, сертификат, подарки, подарок

`company`, `seller` - компания, продавец

`client`, `customer` - клиент, покупатель

`add`, `remove` - добавить, удалить

### Состояния для магазина

`available`, `not-available` - доступен, недоступен
